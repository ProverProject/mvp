...
if (@$_GET['TEST_SEARCHSWYPECODE_PASSWORD'] !== TEST_SEARCHQRCODE_PASSWORD) {
    echo 'please, send GET or configure config.json: TEST_SEARCHSWYPECODE_PASSWORD';
    exit(1);
}
...
if (!empty($_FILES['file'])) {
    ...
    $result = uploadResult($workerResult['isSuccess'], $workerResult['transactions'], $workerResult['hash'], $workerResult['error']);

    echo "file: <pre>";
    var_dump($_FILES);
    echo "</pre>";

    echo "swype: " . $result['transactions'][0]['swype'] . "<br>";
    echo "beginSwypeTime: " . $result['transactions'][0]['beginSwypeTime'] . "<br>";
    echo "endSwypeTime: " . $result['transactions'][0]['endSwypeTime'] . "<br>";
    echo "hash: " . $result['hash'] . "<br>";
    echo "error: " . $result['error'] . "<br>";
    echo "debug: " . $result['debug'] . "<br>";
}